...
lambda = 0.3;       %Change this for different lambda values
eps0 = 8.8542e-12;
mu0 = pi*4e-7;
c0 = 1/sqrt(mu0*eps0);
eta0 = sqrt(mu0/eps0);
...
stripWidth = lambda/40;                % Bowtie Antenna
...
% S11plot = csvread("E:\#PHD\#Research Work\HFSS Designs\S11_Bowtie1GHz.csv");
% frequency = 1e9*S11plot(:,1);
% S11 = S11plot(:,2);
...
% I_strip = Yi*stripWidth;     %Bowtie Antenna axial current
% I2_strip = Xi*stripWidth;    %Bowtie Antenna transversal current
% I_strip(:,1) = []; I_strip(:,length(I_strip)) = [];
% I2_strip(:,1) = []; I2_strip(:,length(I2_strip)) = [];
% I_strip1 = I_strip*1e3;
% I_strip2 = I2_strip*1e3;
% 
% % fig2 = figure();
% % plot(yi,[0 abs(I_strip1) 0],'b','LineWidth',1.4); hold on;
% % plot(yi,[0 abs(I_strip2) 0],'r','LineWidth',1.4);
% % ylabel('Magnitude of Current (mA)','FontSize',14)
% % xlabel('Antenna length, m','FontSize',14)
% % xlim([-stripLength/2 stripLength/2])
% % legend('Axial Component','Transversal Component','Location','northwest','FontSize',14);
% 
% fig2 = figure();
% yyaxis left
